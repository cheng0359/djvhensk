...
function nowPoint(elementId){
    const element = document.getElementById(elementId);
    const currentRect = element.getBoundingClientRect();
    let currentX = currentRect.left; 
}
let halfWidth=(window.innerWidth)-500
let getright=false
let currentFrame = 0;
...
window.addEventListener('wheel', (event) => {
    ...
    if (event.deltaY < 0&&getright==false&&canChangeAnime==true)  {
        ...
        if((positionX-moveSpeedX)<0){
            positionX =0; // 
            console.log("now"+positionX)
            canvas_earth.style.position='absolute'
            canvas_earth.style.top='300%'
